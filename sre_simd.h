...
static inline __simd128_float simd128_set_first_float(float f) {
    return _mm_set_ss(f);
}
...
// Convert the lower two elements to a double occupying the lower and upper half
// of the return value.
...
static inline __simd128_float simd128_load_and_set_w(const Vector3D *v) {
    ...
    if (sizeof(Vector3D) == 16) {
        ...
        __simd128_int m_mask_012x = simd128_shift_right_bytes_int(
            simd128_set1_int32(0xFFFFFFFF), 4);
...
static inline __simd128_float simd128_load_and_set_w(const Point3D *p) {
    ...
    // Set w to 1.0f.
    __simd128_float m_ones = simd128_set1_float(1.0f);
    __simd128_int m_mask_012x = simd128_shift_right_bytes_int(
        simd128_set1_int32(0xFFFFFFFF), 4);
    return simd128_cast_int_float(simd128_or_int(
        simd128_and_int(m_mask_012x, simd128_cast_float_int(m_v)),
...
static char sre_simd_internal_bit_count4[16] = {
...
const Vector4D * __restrict v1, const Vector4D& __restrict v2, float * __restrict dot) {
    ...
    for (; i  + 3 < n; i += 4) {
        ...
        __simd128_float m_v1_0 = simd128_load(&v1[i + 0]);
        __simd128_float m_v1_1 = simd128_load(&v1[i + 1]);
        __simd128_float m_v1_2 = simd128_load(&v1[i + 2]);
        __simd128_float m_v1_3 = simd128_load(&v1[i + 3]);
        __simd128_float m_v1_x, m_v1_y, m_v1_z, m_v1_w;
    ...
    for (; i < n; i++) {
        __simd128_float m_v1 = simd128_load(&v1[i]);
...
const Vector3D * __restrict v1, const Vector3D& __restrict v2, float * __restrict dot) {
    // Load v2 and make sure the w component is set to 0.0f.
    __simd128_float m_v2 = simd128_load_and_set_w(&v2);
    __simd128_float m_v2_x, m_v2_y, m_v2_z;
    m_v2_x = simd128_select_float(m_v2, 0, 0, 0, 0);
    m_v2_y = simd128_select_float(m_v2, 1, 1, 1, 1);
    m_v2_z = simd128_select_float(m_v2, 2, 2, 2, 2);
    ...
    for (; i < n; i++) {
        __simd128_float m_v1 = simd128_load_and_set_w(&v1[i]);
        __simd128_float m_dot = simd128_mul_float(m_v1, m_v2);
        __simd128_float m_result = simd128_horizontal_add4_float(m_dot);
        simd128_store_first_float(&dot[i], m_result);
    }
